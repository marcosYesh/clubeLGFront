...
function btnCheckin() {
    ...
    $('.row-children').each(function (index, value) {
        dataInputArray.push({
            ...
            'BabyCarriageCode': $('#StoreBabyCarriageAvailable option:selected').val(),
    ...
    $.each(dataInputArray, function (i, item) {
        if (validateCheckinChildren(isCpf, dataInputArray[i]) == false) {
            isValid = false;
        }
    });
...
function fillInputFields(playId, returnData, isChange) {
    ...
    searchChildrenByResponsibleCpf(returnData.UserMobileCPF, function (subscriptions) {
        ...
        $('#StoreBabyCarriageAvailable').empty();
        $('#StoreBabyCarriageAvailable').append('<option value="' + returnData.BabyCarriageId + '">' + returnData.BabyCarriageId + '</option>');
        srvGetAvailableBabyCarriage(selectedStoreId, function (data) {
            $.each(data, function (index, value) {
                $('#StoreBabyCarriageAvailable').append('<option value="' + value.CarriageCode + '">' + value.CarriageCode + '</option>');
            });
        });
...
function srvCheckIn(dataInput, callback) {
    $.ajax({
        url: baseUrl + '/api/babycarriageplayground/checkin',
        type: 'POST',
        dataType: 'json',
...
function srvStartCheckin(id, callback) {
    $.ajax({
        url: baseUrl + '/api/babycarriageplayground/startCheckin',
        type: 'POST',
        dataType: 'json',
        data: {
            'PlaygroundId': id,
...
function srvGetAvailableBabyCarriage(storeId, callback) {
    $.ajax({
        url: baseUrl + '/api/babycarriageplayground/getAvailable?storeId=' + storeId,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
            callback(data.ReturnData);
...
function validateCheckinChildren(isCpf, dataInput) {
    if (dataInput.ChildName != '') {
        ...
        $.notify("Campo 'Nome da Participante' é obrigatório", "error");
    }

    return false;
}

function validateCheckinAdult(isCpf, dataInput) {
    if (dataInput.CPF != '') {
        return true;
    } else {
        $.notify("Campo 'Doc. do Responsável' é obrigatório", "error");
