...
function loadAll() {
    getAllCurrent(selectedStoreId, function (dataArray) {
        ...
        $('.table-playgound-body').prepend('<tr class="grid-row"><td class="grid-cell" colspan="14" style="padding: 5px 0;"><a onclick="openCheckinModal()" style="text-decoration: none !important; color: #000; width: 100%; height: 45px; padding: 13px 0" class="btn btn-xs btn-primary btn-checkin"</a>ALUGAR CARRINHO<i class="fa fa-sign-in" style="margin-left:5px;"></i></td></tr>');
        $.each(dataArray, function (index, value) {
        ...
        srvGetAvailableBabyCarriage(selectedStoreId, function (data) {
            $('.availableBabyCarriage').text(data.length)
        });
    });
}

function getAllCurrent(partyBuffetId, callback) {
    $.ajax({
        url: baseUrl + '/api/babycarriageplayground/getAllCurrent?partyBuffetId=' + partyBuffetId,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
...
function getByPlaygroundId(id, callback) {
    $.ajax({
        url: baseUrl + '/api/babycarriageplayground/Get?id=' + id,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
            return callback(data.ReturnData);
...
function getByPlaygroundIdByCPF(id, callback) {
    $.ajax({
        url: baseUrl + '/api/babycarriageplayground/GetByCPF?id=' + id,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
            return callback(data.ReturnData);
...
function postPausetime(playgroundId, callback) {
    $.ajax({
        url: baseUrl + '/api/babycarriageplayground/pause',
        type: 'POST',
        dataType: 'json',
        data: {
...
function postPlaytime(playgroundId, callback) {
    $.ajax({
        url: baseUrl + '/api/babycarriageplayground/start',
        type: 'POST',
        dataType: 'json',
        data: {
            'PlaygroundId': playgroundId,
...
function insertRow(data, first) {
    ...
    var row = '<tr class="grid-row ' + rowColor + '">' +
                ...
                '</td>' +
                '<td class="grid-cell" style="cursor: pointer;"  title="Identifação do carrinho" onclick=openCheckinEditModal(\'' + data.Id + '\')>' + data.BabyCarriageId + '</td>' +
                ...
                (data.Responsible != null && data.IsPaused == false && data.DtEntraceEnd != null ?
                    '<td class="grid-cell"><a class="btn btn-xs btn-primary btn-playground-action" title="Realizar checkout deste aluguél" onclick="openCheckoutModal(' + data.Id + ')"><i class="fa fa-dollar"></i></a></td>' :
