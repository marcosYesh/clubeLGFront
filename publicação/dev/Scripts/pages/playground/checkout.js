...
function btnCheckout() {
    ...
    if (dataInput.DtEntraceEnd != '') {
        if (dataInput.PaymentType > 0) {
            if (dataInput.PaidCost != '') {
                if ($('#PaymentTypeSplit').val() == '1' || $('#PaymentTypeSplit').val() == '2' && dataInput.PaymentTypeSplit > 0 ) {
                    ...
                    $.notify("Campo 'Forma de Pagamento B' deve ser preenchido", "error");
                }
            } else {
                $.notify("Campo 'Valor Cobrado' é obrigatório", "error");
            }
        } else {
            $.notify("Campo 'Forma de Pagamento A' é obrigatório", "error");
        }
    } else {
...
function srvCheckOut(dataInput, callback) {
    ...
    $.ajax({
        url: baseUrl + '/api/playground/checkout',
        type: 'POST',
        dataType: 'json',
        data: dataInput,
...
function srvCalculateCost(id, userSubscriptionId, callback) {
    $.ajax({
        url: baseUrl + '/api/playground/calculateCost?id=' + id + '&userSubscriptionId=' + userSubscriptionId,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
            callback(data.ReturnData);
...
function srvUpdateUserStorePriceLabel(id, label, callback) {
    $.ajax({
        url: baseUrl + '/api/playground/updateUserStorePriceLabel',
        type: 'POST',
        dataType: 'json',
        data: {
            'playgroundId': id,
