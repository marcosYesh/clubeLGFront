...
function btnCheckin() {
    ...
    $('.row-children').each(function (index, value) {
        dataInputArray.push({
            ...
            'PartyBuffetId': selectedPartyBuffetId,
    ...
    if (eventType == 2) {
        ...
        if (validateCheckinChildren(isCpf, dataInputArray[0]) == false) {
            isValid = false;
        }
    } else {
        $.notify("Campo 'Tipo de Entrada' é obrigatório", "error");
        isValid = false;
    }
...
function srvCheckIn(dataInput, callback) {
    $.ajax({
        url: baseUrl + '/api/eventplayground/checkin',
        type: 'POST',
        dataType: 'json',
...
function srvStartCheckin(id, callback) {
    $.ajax({
        url: baseUrl + '/api/eventplayground/startCheckin',
        type: 'POST',
        dataType: 'json',
        data: {
            'PlaygroundId': id,
