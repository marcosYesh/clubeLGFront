LGApp.controller("ProductsController", function ($scope, $rootScope, $cookies, $location, $timeout, video, apiService, $sce, $http, $interval, $routeParams, $route) {
		...
		$timeout(function () {
            $rootScope.userAuthWithSessionId(function (userResponse) {
                ...
                function startWatchers() {
					try {
						...
						$rootScope.$watch('youtubePlayer', function () {
							...
							if($rootScope.youtubePlayer != undefined && $rootScope.youtubePlayer.b == null) {
								setCurrentWatchingVideo($scope.currentVideoId, false);
							}
							else if ($rootScope.youtubePlayer != undefined) {
								console.log($rootScope.youtubePlayer);
								$scope.youtubePlayer = $rootScope.youtubePlayer;
								enableUserToStartVideo();
							}
				...
				function enableUserToStartVideo() {
					$timeout(function () {
                ...
                function setCurrentWatchingVideo(videoId, isReplay) {
                    if(!isReplay) { $rootScope.currentVideoIsLoading = true; }
                ...
                function setCurrentWatchingVideoQuestions(video) {
                    //added raw time for product video questions ordering [ABREU 18-10-2017]
                    angular.forEach(video.ProductVideoQuestionList, function (v, index) {
						video.ProductVideoQuestionList[index].ShowtimeRaw = parseInt((v.Showtime.split(':')[0] * 60) + (v.Showtime.split(':')[1]));
                    });
					...
					if ($scope.currentWatchingVideoQuestions.VideoHasQuestions) {
						if ($rootScope.currentTime != undefined) {
							angular.forEach($scope.currentWatchingVideoQuestions, function (videoQuestion, videoQuestionIndex) {
								...
								if (hasQuestion == false && videoQuestion.Showtime != "REMOVED" && $rootScope.currentTime > 0) {
					...
					if ($rootScope.currentTime != undefined && $rootScope.duration != undefined && $rootScope.duration > 0) {
						if ($rootScope.currentTime >= $rootScope.duration) {
							if (!$scope.currentWatchingVideo.videoFinished) {
								apiService.markVideoWatched($scope.currentWatchingVideo.Id, userResponse.Token).then(function (response) {
									loadCurrentProductPercentageOnly($routeParams.productId);
									/*
									apiService.getProductById($routeParams.productId, userResponse.Token).then(function (response) {
										if (response != undefined && response.data.ResultStatus == 'SUCCESS') {
											
											$scope.currentWatchingVideo.ProductVideoQuestionList = [];
											
											setCurrentProduct(response.data.ReturnData.Product);
											setCurrentWatchingVideo($scope.currentVideoId, true);
							...
							apiService.getTotalPrize(userResponse.Token).then(function (response) {
								if (response.data.ResultStatus != 'ERROR') {
									$rootScope.userProfile.TotalPrize = parseInt(response.data.ReturnData);
									console.log($rootScope.userProfile.TotalPrize)
								}
							});
                ...
                function loadCurrentProduct(productId) {
                    ...
                    apiService.getProductById(productId, userResponse.Token).then(function (response) {
                        if (response != undefined && response.data.ResultStatus == 'SUCCESS') {
                            ...
                            $rootScope.$watch('currentTime', watchShowQuestion);
                ...
                $scope.playTheVideo = function (video) {
				...
				$scope.replayTheVideo = function (video) {
                    $scope.changecurrentWatchingVideo(video);
                }
                ...
                $scope.rateQuizExperience = function () {
                    if (!$scope.ratingLoading) {
                        ...
                        apiService.quizRate($scope.currentWatchingVideo.Id, ratingAmount, userResponse.Token).then(function (response) {
							...
							// TODO: tentativa de correção de bug (loadCurrentProduct($routeParams.productId);)
							// changecurrentWatchingVideo($scope.currentVideoId);
                        });
                    }
                }
                ...
                $scope.answerQuiz = function (answer, event, answerType, index) {
                    ...
                    if (!$scope.blockAnswerClick) {
						...
						// TODO: inserir REMOVED aqui
						$scope.currentWatchingVideo.ProductVideoQuestionList[index].Showtime = "REMOVED";
                    }
                }
